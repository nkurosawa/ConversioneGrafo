...
for graph_dict in elem.getiterator("graph"):
    ...
    plt.subplots_adjust(top=0.9,bottom=0,hspace=1)

    plt.suptitle('Date qualities by detector. (I/sigma > 0.0, excl. dets = 4.14)', size=15)
    ...
    plt.xticks(X_labels,X_labels_value,size=10)
    plt.margins(0.01,0)
    lgnd = plt.legend(loc="upper left", bbox_to_anchor=(-0.005,1.9), ncol=1)
    plt.grid()
